...
let tryGetArrayIndex (index: int) (xs: 'T array) : 'T option =
    if index >= xs.Length then None else Some xs[index]
...
let rec executeInstructionPartOne
    (stacks: ImmutableStack<string> array)
    (instruction: Instruction)
    : (ImmutableStack<string> array) =
...
let executeInstructionPartTwo
    (stacks: ImmutableStack<string> array)
    (instruction: Instruction)
    : (ImmutableStack<string> array) =
    printf $"Running instruction {instruction}\n\n"

    let (crates, newSource) =
        stacks[ instruction.Source ].PopMany instruction.NumberOfCrates
...
let input =
    File.ReadAllText("./05/input.txt") |> String.split [ "\n\n" ] |> Seq.toArray
...
let execute executeInstruction () =
    ...
    let result = finalStacks |> Seq.map (fun stack -> stack.Peek()) |> String.concat ""
