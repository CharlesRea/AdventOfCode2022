...
module Part1 =
    let parseCompartment (compartment: string) = compartment |> seq |> Set.ofSeq
    ...
    let scoreInputLine (line: string) =
        let duplicates =
            line |> parseLine |> (fun (x, y) -> Set.intersect x y) |> Set.toList
    ...
    let result () =
        File.ReadLines("./03/input.txt") |> Seq.sumBy scoreInputLine

module Part2 =
    let scoreGroup (group: string seq) =
        let duplicates =
            group |> Seq.map (seq >> Set.ofSeq) |> Set.intersectMany |> Seq.toList
    ...
    let result () =
        File.ReadLines("./03/input.txt") |> Seq.chunkBySize 3 |> Seq.sumBy scoreGroup
